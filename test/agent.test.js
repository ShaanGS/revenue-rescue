...
test('does not execute enterprise work before approval', async () => {
  const result = await executeRecovery(planRecovery(acme, 'run-1'), createDemoConnectors());
  assert.equal(result.status, 'awaiting_approval');
  assert.equal(result.receipts.length, 0);
});

test('executes and verifies five recovery writes after approval', async () => {
  const result = await executeRecovery(planRecovery(acme, 'run-2'), createDemoConnectors(), { approved: true });
...
test('fails closed if a connector cannot verify its write', async () => {
  const connectors = createDemoConnectors({ slack: { failVerificationFor: 'post_escalation' } });
  const result = await executeRecovery(planRecovery(acme, 'run-3'), connectors, { approved: true });
