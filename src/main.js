...
function render() {
  ...
  app.innerHTML = `
    <main>
      <nav><div class="brand"><span class="mark">R</span> RevenueRescue</div><div class="status"><span></span> Sandbox connectors online</div></nav>
      <section class="hero">
  ...
  document.querySelector('#toggle').onclick = () => { activeAccount = activeAccount.doNotContact ? acme : protectedAccount; approved = false; executed = false; render(); };
  document.querySelector('#approve')?.addEventListener('click', () => { approved = true; render(); });
  document.querySelector('#execute')?.addEventListener('click', () => { executed = true; render(); });
}
render();
